...
parseArgs = do
    args <- getArgs
    case args of
        [coreCfgFile, skeyFile, oref, cs, tn, oldRate] -> do
            let tokenName = tokenNameFromHex $ pack tn
                cs' = fromString cs
                oldRate' = read oldRate :: Integer
            case tokenName of
                Right tn' -> return (coreCfgFile, skeyFile, fromString oref, cs', tn', oldRate')
...
main :: IO ()
main = do
    (coreCfgFile, skeyFile, oref, cs, tn, oldRate) <- parseArgs
    ...
    withCfgProviders coreCfg "oracle" $ \providers -> do
        ...
        case utxoM of
            Just utxo -> do
                    txBody <- runGYTxMonadNode nid providers [sender] sender Nothing (return $ deleteOracle cs tn sender (utxoRef utxo) oldRate)
